...
interface Props {
  ...
  track: ReadonlyArray<string | number>;
  label?: string;
  className?: string;
}
...
*/
export default forwardRef<HTMLUListElement, Props>(function PickerWheel({ list, wheel, track, label, styles, className }, ref) {
  return (
    <div className={`${styles["picker-control"]} ${styles[`picker-control--${className}`]} ${className && styles[className]}`}>
      {/* 화면에 리스트 내용은 노출되지 않으나, 실제 사용자의 스크롤을 감지하는 영역  */}
      <ul ref={ref} className={`${styles["picker-controller"]} ${styles[`picker-controller--${className}`]}`}>
        {list.map((value, idx) => (
          <li key={idx}>{value}</li>
        ))}
      </ul>
      ...
      <div className={`${styles["picker-wheel"]} ${styles[`picker-wheel--${className}`]}`}>
        {wheel.map((value, idx) => (
      ...
      {/* 스크롤 가운데 바(::after)에 강조(하이라이트)되서 노출되는 데이터 영역 */}
      <div className={`${styles["picker-track"]}`}>
        <div className={`${styles["picker-track-holder"]} ${styles[`picker-track--${className}`]}`}>
          {track.join(" ")}
        </div>
      </div>
