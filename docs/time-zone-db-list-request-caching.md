...
> 
> 본 문서는 세계 도시 시계 기능 구현을 위해 공용 API(Open API)인 **Time Zone DB를 활용하게 된 배경**과, World 라우트로 이동할 때마다 **List Time Zone API가 동일한 데이터임에도 반복적으로 요청되던 문제**를 **분석하고 성능을 개선하기 위해 캐싱 과정을 도입한 내용을 정리한 문서**입니다.
>
...
>
> 그럼에도 불구하고 **본 문서를 삭제하지 않고 유지하는 이유**는, **브라우저 캐시를 활용할 수 없는 상황에서도 다른 저장소를 통해 캐싱을 도입하여 성능을 개선할 수 있는 방법을 분석한 과정**과, **문제를 인지하고 개발자 도구를 활용해 렌더링 흐름을 분석하여 해결 방안을 도출한 문제 해결 과정을 함께 기록**하고 있기 때문입니다. 또한 개발에는 정답이 존재하기 않기 때문에, **상황에 맞는 해결 전략을 선택하는 과정 자체가 의미 있다고 판단**하여 **해당 문서를 그대로 보존하기로 결정**하였습니다.
...
Local Storage에 List Time Zone API의 응답 결과가 포함되어 있는가?
GET Cached(local storage): 0.25390625 ms
```
...
FPS 변화 과정을 측정한 비디오를 통해 확인해보면, **동기 방식과 비동기 방식이 렌더링 흐름에 미치는 영향의 차이**가 명확하게 드러납니다.

먼저 **동기 방식인 Local Storage**의 경우, 데이터 조회 과정에서 **JavaScript 메인 스레드를 직접 점유**하게 됩니다. 이로 인해 기존에 **120~140 FPS를 유지하던 상태**에서도 **World 페이지로 이동하는 시점에 FPS가 80~90 수준까지 급격히 감소하는 현상이 발생**하는 것을 확인할 수 있습니다.

반대로 **비동기 방식인 IndexedDB**는 데이터 조회 과정에서 JavaScript 메인 스레드를 점유하지 않고, **실제 처리 작업을 브라우저 내부의 백그라운드 스레드에서 수행**합니다. 그 결과, World 페이지로 이동하더라도 **기존의 120~140 FPS 수준이 안정적으로 유지**되는 것을 확인할 수 있습니다.

이러한 결과를 종합해보면, `console.time`을 통해 **출력된 시간만을 기준으로 Local Storage가 더 빠르다고 판단할 경우**, 실제 사용자 환경에서는 **프레임 드랍과 같은 성능 저하를 직접적으로 체감**하게 될 수 있습니다. 반면 **IndexedDB**는 비동기 처리 특성항 콘솔에 출력되는 전체 시간만 놓고 보면 오래 걸리는 것처럼 보이지만, **실제로는 메인 스레드를 점유하지 않기 때문에 렌더링 성능은 훨씬 안정적으로 유지**됩니다.

따라서 브라우저 캐시 저장소를 대체하기 위한 저장소로서 Local Storage와 IndexedDB를 비교했을 때, 단순 처리 시간뿐만 아니라 **JavaScript 메인 스레드 점유 여부**와 **실제 FPS 변화를 함께 고려한 결과**, **IndexedDB가 더 적절한 선택**이라고 판단할 수 있었습니다. 이에 Clock 프로젝트에서는 IndexedDB를 활용하여 기존 브라우저 캐시 저장소를 대체하기로 결정했습니다.