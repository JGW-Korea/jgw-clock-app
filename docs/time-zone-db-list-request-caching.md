...
먼저 **동기 방식인 Local Storage**의 경우, 데이터 조회 과정에서 **JavaScript 메인 스레드를 직접 점유**하게 됩니다. 이로 인해 기존에 **120~140 FPS를 유지하던 상태에서도 World 페이지로 이동하는 시점에 FPS가 80~90 수준까지 급격히 감소하는 현상이 발생**하는 것을 확인할 수 있습니다.
...
따라서 브라우저 캐시 저장소를 대체하기 위한 저장소로서 Local Storage와 IndexedDB를 비교했을 때, 단순 처리 시간뿐만 아니라 **JavaScript 메인 스레드 점유 여부**와 **실제 FPS 변화를 함께 고려한 결과**, *8IndexedDB가 더 적절한 선택**이라고 판단할 수 있었습니다. 이에 Clock 프로젝트에서는 IndexedDB를 활용하여 기존 브라우저 캐시 저장소를 대체하기로 결정했습니다.